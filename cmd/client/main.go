...
func main() {
	...
	prov, err := buildProvider(ctx, cfg, appDialer, &connectedStreams, logger)
	if err != nil {
	...
	// параллельных TURN-аллокаций => полосы аккаунтов складываются).
...
func buildProvider(ctx context.Context, cfg *config.Client, dialer net.Dialer, connected *atomic.Int32, logger logx.Logger) (provider.Provider, error) {

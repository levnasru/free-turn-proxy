...
- **Перезапись только при реальном изменении содержимого**, не безусловно на
  каждом старте: у `client`/`xray`/`vkturn-desktop` версии не синхронизированы
  друг с другом (десктоп — свой хардкод `1.0.0`, client — тег репо, xray —
  апстримная версия xray-core), так что единой строки "версия" сравнивать не
  с чем — вместо неё sha256 от самих embedded-байт как маркер. Функция
  `extractIfChanged(path string, data []byte, perm os.FileMode) error`:
  считает `sha256.Sum256(data)`, сверяет с sidecar-файлом `<path>.sha256`
  (пишется рядом при экстракции); хеш совпал и целевой файл на месте →
  пропустить запись; иначе — записать файл и обновить sidecar. Работает и
  как "версия не изменилась — не трогаем" (типичный случай, экономит запись
  15-80MB на каждый обычный старт), и как "обновили vkturn-desktop —
  embedded-байты другие — hash не совпал — перезаписываем" автоматически, без
  отдельного шага синхронизации версий при релизе.
...
- Юнит-тесты на `extractIfChanged()`: (1) пишет во временную директорию (не в
  реальный `~/.vkturn/`), проверяет, что байты на диске совпадают с входными
  данными и файл исполняем (`0o755`) на Linux; (2) второй вызов с теми же
  данными — файл не перезаписывается (проверить через mtime или
  подмену данных на заведомо отличающиеся и обратно); (3) вызов с другими
  данными — перезаписывается, sidecar-хеш обновляется.
- Ручной golden-path (как в предыдущем плане): собрать двухпроходной сборкой
  ...
  `~/.vkturn/bin/` наполняется и подпроцессы стартуют оттуда; перезапустить
  `vkturn-desktop` второй раз без пересборки — убедиться, что `client`/`xray`
  не перезаписываются (mtime не меняется).

## Открытые риски

- Первый запуск после установки/обновления версии будет чуть медленнее
  (запись ~50-80MB на диск), последующие — нет благодаря
  `extractIfChanged()`. Не измерено, вероятно не критично на SSD, но стоит
  замерить на реальной машине.

...
func Listen(addr *net.UDPAddr, key []byte) (dtlsnet.PacketListener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("rtpvideo: udp listen: %w", err)
	}
...
func (l *packetListener) Accept() (net.PacketConn, net.Addr, error) {
	...
	// 3ms scheduling delay to prevent pion/dtls race on freshly accepted conn
...
func (c *packetConn) ReadFrom(p []byte) (int, net.Addr, error) {
	...
	if len(wire) < opusLegacyOverhead {
		return 0, addr, errors.New("rtpvideo: packet too short")
	}
